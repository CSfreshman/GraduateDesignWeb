<template>
  <div class="studentList" v-if="role > 1">
    ...
    <el-form :inline="true" :model="formInLine" class="demo-form-inline" size="small">
      <el-form-item>
        <el-input v-model="formInLine.stuNo" placeholder="请输入学号"></el-input>
      </el-form-item>
      <el-form-item>
    ...
    <el-dialog :title="isAdd ? '新增学生信息' : '修改学生信息' " :visible.sync="dialogFormVisible">
      <el-form :model="student">
        ...
        <el-form-item label="学号" :label-width="formLabelWidth">
          <el-input v-model="student.stuNo" autocomplete="off"></el-input>
        </el-form-item>
        <!--        姓名-->
...
export default {
  ...
  methods: {
    ...
    //获取所有数据
    getData(){
      this.service.get('/studentInfo/getAll',this.student)
          .then(res=>{
            console.log(res)
    ...
    del(row){
      this.$confirm('此操作将删除学生信息以及对应的成绩信息, 是否继续?', '提示', {
        ...
        console.log("执行删除方法")
        console.log(row)
        this.service.post("/studentInfo/delete", row)
            .then(res=>{
              if(res.data.code == 200){
                this.$message({
                  type: 'success',
    ...
    findStu(){
      ...
      this.service.post("/studentInfo/getOne",this.formInLine)
          .then(res=>{
    ...
    submitAdd(){
      ...
      if(this.isAdd){
        ...
        this.service.post("/studentInfo/add",this.student)
            .then(res=>{
              console.log(res)
              if(res.data.code === 200){
                this.$message({
        ...
        console.log("执行修改方法")
        this.service.post("/studentInfo/update",this.student)
            .then(res=>{
              console.log(res)
              if(res.data.code === 200){
                this.$message({
